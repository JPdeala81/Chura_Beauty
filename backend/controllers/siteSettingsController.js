...
const siteSettingsController = {
  ...
  getQRCodeConfig: async (req, res) => {
    try {
      const { data, error } = await supabase
        .from('site_settings')
        .select('qr_code_config')
        .single()
      ...
      const config = data?.qr_code_config || {
        enabled: false,
        type: 'info',
        ussdCode: null,
        phoneNumber: null
  ...
  updateQRCodeConfig: async (req, res) => {
    try {
      const { enabled, type, ussdCode, phoneNumber } = req.body

      // Validate required fields
      if (typeof enabled !== 'boolean' || !['info', 'call'].includes(type)) {
        return res.status(400).json({ 
          error: 'enabled (boolean) and type (info|call) are required' 
        })
      }

      // If type is 'call', ensure either ussdCode or phoneNumber is provided
      if (type === 'call' && !ussdCode && !phoneNumber) {
        return res.status(400).json({ 
          error: 'ussdCode or phoneNumber required when type is call' 
        })
      }
      ...
      const config = {
        enabled,
        type,
        ussdCode: type === 'call' ? ussdCode : null,
        phoneNumber: type === 'call' ? phoneNumber : null,
      ...
      if (existing) {
        result = await supabase
          .from('site_settings')
          .update({ qr_code_config: config })
          .eq('id', existing.id)
          .select('qr_code_config')
      } else {
        result = await supabase
          .from('site_settings')
          .insert([{ qr_code_config: config }])
          .select('qr_code_config')
      }
      ...
      res.json({ 
        message: 'QR Code configuration updated', 
        config: data[0]?.qr_code_config 
      })
    } catch (err) {
      res.status(500).json({ error: err.message })
    }
  ...
  // Enable QR Code
  enableQRCode: async (req, res) => {
    try {
      const { type, ussdCode, phoneNumber } = req.body

      if (!['info', 'call'].includes(type)) {
        return res.status(400).json({ error: 'type must be info or call' })
      }
      ...
      res.json({ message: 'QR Code enabled', config: data[0]?.qr_code_config })
    } catch (err) {
      res.status(500).json({ error: err.message })
    }
  },
  ...
  // Disable QR Code
  disableQRCode: async (req, res) => {
    try {
      const { data: existing } = await supabase
        .from('site_settings')
        .select('id, qr_code_config')
        .single()
      ...
      const config = {
        enabled: false,
        type: existing.qr_code_config?.type || 'info',
        ussdCode: existing.qr_code_config?.ussdCode || null,
        phoneNumber: existing.qr_code_config?.phoneNumber || null,
        updatedAt: new Date().toISOString()
      }
      ...
      res.json({ message: 'QR Code disabled', config: data[0]?.qr_code_config })
    } catch (err) {
      res.status(500).json({ error: err.message })
    }
  }
